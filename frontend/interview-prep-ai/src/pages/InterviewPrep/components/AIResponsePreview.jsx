			...
			<div className='text-[14px] prose prose-slate dark:prose-invert max-w-none'>
				<ReactMarkdown
					...
					components={{
						...
						blockquote({ children }) {
							return (
								<blockquote className='border-l-4 border-gray-200 pl-4 italic m-4'>
									{children}
								</blockquote>
							)
						},
						...
						tbody({ children }) {
							return (
								<tbody className='divide divide-gray-200'>{children}</tbody>
							)
						},
						tr({ children }) {
...
function CodeBlock({ code, language }) {
    ...
    <div className='flex items-center justify-between px-4 py-2 bg-gray-100 border-b border-gray-200'>
      ...
      <button onClick={copyCode} className='text-gray-500 hover:text-gray-700 focus:outline-none relative group' aria-label='Copy Code'>
        ...
        {
          copied ? (<LuCheck size={16} className='text-green-600'/>) : (<LuCopy size={16} className=''/>)
        }
        {
          copied && (
